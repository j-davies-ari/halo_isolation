...
from apertures import create_apertures
...
class Snapshot_Volume(object):
    def __init__(self, centers, side_sizes, bulkvels, central_gnums,sgnums,
        ...
        if overwrite == True or not os.path.exists(vol_file):    
            ...
            for t in part_types:
                ...
                posarr = load_array('Coordinates', t, array_type=pdata_type, run=run, model=model, tag=tag)
                '''
                masks = []
                grouppos = []
                ...
                grouppos, masks = create_apertures(posarr,centers,side_sizes,boxsize)
    ...
    def save_individuals(self, path='/data5/arijdav1/saved_regions/', overwrite=True):      ###### For saving each subhalo as a seperate file
